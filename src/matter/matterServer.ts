  ...
  private async registerAccessory(pluginIdentifier: string, platformName: string, accessory: MatterAccessory): Promise<void> {
    ...
    try {
      ...
      if (accessory.handlers) {
        ...
        if (deviceType === devices.RoboticVacuumCleanerDevice || (deviceType as any).name === 'RoboticVacuumCleaner') {
        ...
        for (const clusterName of Object.keys(accessory.handlers || {})) {
          ...
          if (clusterName === 'windowCovering' && (accessory.context as any)?._skipWindowCoveringBehavior) {
          ...
          if (clusterName === 'windowCovering') {
            ...
            if (behaviorClass && windowCoveringFeatures && windowCoveringFeatures.length > 0) {
              behaviorClass = (behaviorClass as any).with(...windowCoveringFeatures)
      ...
      if (accessory.parts && accessory.parts.length > 0) {
        ...
        for (const part of accessory.parts) {
          ...
          if (part.handlers) {
            ...
            const partBehaviorMap: Record<string, BehaviorType> = {
              onOff: HomebridgeOnOffServer,
              levelControl: HomebridgeLevelControlServer,
              colorControl: HomebridgeColorControlServer,
              windowCovering: HomebridgeWindowCoveringServer,
            ...
            if (partCustomBehaviors.length > 0) {
              ...
              partDeviceType = (partDeviceType as typeof partDeviceType & { with: (...behaviors: BehaviorType[]) => EndpointType }).with(...partCustomBehaviors)
          ...
          if (!this.config.externalAccessory) {
            // Matter.js device types support .with() to add behaviors
            partDeviceType = (partDeviceType as typeof partDeviceType & { with: (...behaviors: BehaviorType[]) => EndpointType }).with(BridgedDeviceBasicInformationServer)
          }
