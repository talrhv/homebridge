...
function convertBehaviorsToArray(behaviors: unknown): BehaviorInfo[] {
  ...
  try {
    return Array.from(behaviors as Iterable<BehaviorInfo>)
  } catch {
    return []
  }
}
 ...
 */
function findBehaviorByCluster(
  behaviors: BehaviorInfo[],
  clusterIdOrName: number | string,
): BehaviorInfo | undefined {
...
export function applyWindowCoveringFeatures(
  ...
  const modifiedDeviceType = (deviceType as any).with(
    (HomebridgeWindowCoveringServer as any).with(...features),
  )
